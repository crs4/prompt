...
def line_circle_intersection(arc, circle_pos, circle_radius, inset=True):
    ...
    if p1 != p2:
        ...
        if arc.r_a != arc.r_b:
            delta = sqrt(
                arc.r_b ** 2 * (arc.r_a ** 4 - arc.r_a ** 2 * arc.r_b ** 2 + arc.r_b ** 2 * circle_radius ** 2))
            y = ((circle_pos[1] - circle_radius) * arc.r_a ** 2 + delta - arc.r_b ** 2 * circle_pos[1]) / (
                arc.r_a ** 2 - arc.r_b ** 2)
            if inset:
...
class spring:
    ...
    def draw(self, surface):
        ...
        if self.source_pos != self.target_pos:
            ...
            dy = -sqrt((1 - 1 / self.r_a ** 2) * self.r_b ** 2 * (inter2[0] - ellipse_center[0]) ** 2) / (
                inter2[0] - ellipse_center[0])
            tangent = vec2d(1, dy)
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_ESCAPE:
                    ...
                    for z in self.nodes:
                        z.x = vec2d(uniform(self.w / 2 - 10, self.w / 2 + 10),
                                    uniform(self.h / 2 - 10, self.h / 2 + 10))
                        z.oldx = z.x
...
def draw(cnet):
    ...
    s = ForceDirectedGraph(nodes, inset, outset)
    s.run()
