...
class TestHeuristicMiner(unittest.TestCase):
    ...
    def test_mine_from_csv_file(self):
        print("Executing test_mine_from_csv_file")
        hm = HeuristicMiner()
        ...
        '''
        hnets = hm.mine_from_csv_file(filepath)
        for key in hnets:
            hnet = hnets[key]
            print hnet.id
            #print hnet.nodes
    ...
    def test_mine(self):
        print("Executing test_mine")
        hm = HeuristicMiner()
        log = self.create_log_from_test_data()
        gnet = hm.mine_dependency_graph(log=log, frequency_threshold=0, dependency_threshold=0.0)
        print gnet.id
        # print hnet.nodes
        for node in gnet.nodes:
            print "Node: " + str(gnet.nodes[node])
        #print hnet.arcs
        for arc in gnet.arcs:
            print "Arc: " + str(gnet.arcs[arc])
        print("============================================================")
        print("")
        hnets = hm.mine(log,frequency_threshold=0,dependency_threshold=0.0)
        for key in hnets:
            hnet = hnets[key]
            print hnet.id
            #print hnet.nodes
            for node in hnet.nodes:
                print "Node: "+str(hnet.nodes[node])
                print "Node - input_binds: "+str(hnet.nodes[node].input_bindings)
                print "Node - output_binds: "+str(hnet.nodes[node].output_bindings)
            #print hnet.arcs
            for arc in hnet.arcs:
                print "Arc: "+str(hnet.arcs[arc])
        print("============================================================")
...
def suite():
    ...
    suite.addTest(TestHeuristicMiner('test_mine_from_csv_file'))
    return suite
